...
\item Optimize the performance with grid adaptation and optionally multi-grid
...
\begin{itemize}\item Unsteady adjoint for a simple  problem with check pointing
...
\begin{itemize}
\item Unsteady explicit Euler solver(Residual Distribution Scheme)
\end{itemize}
\end{block}
}
\uncover<1->{
\begin{block}{Primary Task till December 2014}
\begin{itemize}
\item Implementation of unsteady adjoint in the Euler solver 
%\item Implementation of moving geometry 
\item Gradient-based shape optimization for single objective (expected)
\end{itemize}
\end{block}
}
\end{frame}
...
\begin{itemize}
%\item Planned on Feb-March 2015
...
%\item Develop problem solving skill for cfd related
